...
class HotelsDAL(BaseDAL):
    ...
    async def get_all(
        ...
        async with async_session_maker() as session:
            '''get hotels in this location if rooms_left > 0'''
            """
            ...
            """
            booked_rooms = (select(Bookings.room_id, func.count(Bookings.room_id).label("rooms_booked")).select_from(Bookings)
            .where(
                    or_(
                        and_(
            ...
            booked_hotels = select(Rooms.hotel_id, func.sum(
